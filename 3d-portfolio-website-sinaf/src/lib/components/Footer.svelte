...
<Bounded as="footer">
	...
	>
		...
		>
			<a
				href="/"
				class="text-xl font-extrabold tracking-tighter text-slate-100 transition-color duration-150 hover:text-yellow-200"
			>
				{settings.data.name}
			</a>
			<span
		...
		<nav class="navigation" aria-label="Footer Navigation">
			<ul class="flex items-center gap-1">
				{#each settings.data.nav_items as { link, link_label }, index}
					<li>
						<PrismicLink
							field={link}
							class="block px-3 py-1 text-base font-bold text-slate-100 transition-colors duration-150 hover:text-yellow-200"
							>{link_label}</PrismicLink
						>
					</li>
					{#if index < settings.data.nav_items.length - 1}
						<span class="text-4xl font-thin leading-[0] text-slate-400 inline" aria-hidden="true"
							>/</span
						>
					{/if}
				{/each}
		...
		<div class="socials inline-flex justify-center sm:justify-end">
			{#if isFilled.link(settings.data.github_link)}
				<PrismicLink
					...
					class="p-2 text-2xl text-slate-300 transform transition-all duration-150 hover:text-yellow-200 hover:scale-150"
